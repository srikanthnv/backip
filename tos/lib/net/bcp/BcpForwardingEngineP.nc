...
implementation{
	...
	task void sendBeaconTask() {
		...
		//call BcpDebugIF.reportError(0xA4); 
	...
	task void sendDataTask() {
		...
		if( sendQeOccupied == TRUE )
		{
			...
			if( retVal == FAIL )
			{
				...
				dbg("Routing", "%s %d\n", __FUNCTION__, __LINE__);
				//call BcpDebugIF.reportError( 0xA1 );
				return;
			}
			...
			if( call SendStack.empty() )
			{
				...
				if (nullMsg == NULL) {
					...
					dbg("Routing", "%s %d\n", __FUNCTION__, __LINE__);
					call BcpDebugIF.reportError( 0xA2 );
					return;
				}
		...
		if( qe->txCount >= MAX_RETX_ATTEMPTS )
		{
			...
			sending = FALSE;
			call txRetryTimer.startOneShot( REROUTE_TIME );
			call BcpDebugIF.reportError( 0xA3 );
			dbg("Routing", "%s %d\n", __FUNCTION__, __LINE__);
			return;
		}
		...
		if (subsendResult == SUCCESS) {
			...
			call BcpDebugIF.reportError( 0xA7 );
			return;
		}
	...
	event message_t* SubReceive.receive(message_t* msg, void* payload, uint8_t len)
	{
		...
		latestForwarded_table_entry * latestForwardedEntry;
		uint32_t arrivalTime = call DelayPacketTimer.getNow();
		dbg("BCP", "In BCPForwardingEngine SubReceive.receive\n");
	...
	event message_t* BeaconReceive.receive(message_t* msg, void* payload, uint8_t len)
	{
		 ...
		 */
		call BcpDebugIF.reportError(0xA5 ); 
