		...
		atomic{
			...
			for (compareIdx = 0; compareIdx < routingTableActive; compareIdx++) {
				...
				if( routingTable[compareIdx].isBurstyNow == 1 )
				{
					...
					dbg("Forwarder", "compareIdx %u compareWeight %d"
							" longLocalBackpressure %u longNeighborBackpressure %u"
							" longLinkETX %u longLinkRate %u"
							" maxWeight %d\n",
							compareIdx, compareWeight,
							longLocalBackpressure, longNeighborBackpressure,
							longLinkETX, longLinkRate, maxWeight);
