...
init_entry:
	...
	command route_key_t ForwardingTable.addRoute(const uint8_t *prefix, 
		...
		dbg("ForwardingTable", "ForwardingTable.addRoute\n");
	...
	command error_t ForwardingTable.delRoute(route_key_t key) {
		int i;
		dbg("ForwardingTable", "ForwardingTable.delRoute\n");
		for (i = 0; i < ROUTE_TABLE_SZ; i++) {
	...
	error_t do_send(uint8_t ifindex, struct in6_addr *next, struct ip6_packet *pkt) {
		...
		//dbg("IPForwardingEngineP", "doing send!\n");
		if (iid != NULL)
	...
	command error_t IP.send(struct ip6_packet *pkt) {
		...
		char str[40];
		inet_ntop6(&pkt->ip6_hdr.ip6_dst, str, 40);

		dbg("IPForwardingEngineP", "looking up %s\n", str);
		...
		//dbg("IPForwardingEngineP","Forwarding.\n");
		if (call IPAddress.isLocalAddress(&pkt->ip6_hdr.ip6_dst) && 
			...
			if (pkt->ip6_hdr.ip6_dst.s6_addr[0] != 0xff) {
				//dbg("IPForwardingEngineP","Sending via do_send\n");
				return do_send(ROUTE_IFACE_154, &pkt->ip6_hdr.ip6_dst, pkt);
			} else {
				//dbg("IPForwardingEngineP","Sending directly\n");
			...
			{
				//dbg("IPForwardingEngineP","3a.\n");
				return FAIL;
			}
			//dbg("IPForwardingEngineP","4.\n");

			return do_send(next_hop_entry->ifindex, &next_hop_entry->next_hop, pkt);
		} else {
			dbg("IPForwardingEngineP","5.\n");
		} 
		return FAIL;
	}
	...
	event void IPForward.recv[uint8_t ifindex](struct ip6_hdr *iph, void *payload, 
		...
		if (call IPAddress.isLocalAddress(&iph->ip6_dst)) {
			...
			//dbg("IPForwardingEngineP", "Is local address, signaling event\n");
			memcpy(&pkt.ip6_hdr, iph, sizeof(struct ip6_hdr));
			pkt.ip6_data = &v;
			pkt.ip6_inputif = ifindex;

			/* give the routing protocol a chance to 
				 look at headers on this packet. */
			dbg("BackIPFwdEvent", "local\n");
			if (!(signal ForwardingEvents.approve[ifindex](&pkt, next_hop)))
			{
				dbg("BackIPFwdEvent", "s\n");
			}
			...
			//if (!(--iph->ip6_hlim)) {
				...
				dbg("IPForwardingEngineP", "Hop limit exceeded, dropping\n");
				dbg("DROP", "DROP>>Hop limit exceeded, dropping\n");
				return;
			}
			dbg("IPForwardingEngineP", "Is not local address, forwarding\n");
	...
	default event bool ForwardingEvents.approve[uint8_t idx](struct ip6_packet *pkt,
			struct in6_addr *next_hop) {
		dbg("BackIPFwdEvent", "def approve\n");
		return TRUE;
	}
	...
	default command error_t IPForward.send[uint8_t ifindex](struct in6_addr *next_hop,
			struct ip6_packet *pkt,
			void *data) {
		dbg("IPForwardingEngineP", "In default send, doing nothing!\n");
