...
const static uint32_t SEND_PERIOD   = 1000;
const static uint32_t NUM_NODES     = 48;
const static uint32_t NUM_PACKETS   = 500;
...
void sendCommand( int type_p, int fd_p, uint8_t commandCount_p )
{
  ...
  while( success == 0 )
  {
    ...
    printf("\nSending command to %u,%u\n",nodeID_m, commandCount_p); 
...
int main(int argc, char **argv)
{
  ...
  sendCommand( COMMAND_START, fd, 0 );
